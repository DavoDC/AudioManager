...
- [ ] **Audit: move all critical artist casing rules to config** - The artist-name-overrides.xml system is durable and version-controlled. However, critical casing rules (like "Scott Adams" must stay title-cased, not drift to lowercase) can be scattered in code comments and easily forgotten. Audit the codebase (TagFixer.cs comments, CLAUDE.md notes, HISTORY.md) for any artist casing rules and move them all to artist-name-overrides.xml so the rule lives in config, not comments. This prevents "accidentally blatted" rules.
