...
- [ ] **UX: Fix duplicate detection output layout - show existing + new info grouped together** - Currently duplicate output is hard to read because existing track info and new file info are scattered. When showing a duplicate, display existing track metadata and new file info grouped together with a space between them for clarity. This makes it easier to visually compare the two files at a glance.

- [ ] **UX: Show corrected tag name in duplicate output (not original filename)** - New files in duplicate detection currently show the original filename (e.g. "03 - Angel.mp3") instead of the corrected tag name that will be applied after tag fixing (e.g. "Shaggy;Rayvon - Angel.mp3"). After tag fixing, the filename is secondary - show the final corrected tag so the user sees what will actually be stored.
