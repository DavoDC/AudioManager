...
**Integration prep order (/dev-session: start here): (1) implement gate + (with X) fixer below. (2) use new unified Integration command (dry run -> review -> Y to proceed). (combined summary + progress indicators: done 2026-05-25. unified run log: done 2026-05-25)**

- [ ] **Gate real integration behind dry run (unified integration command)** - Current launch.bat has 4 options: Analysis, Analysis+Regen, Integration (Dry Run), Integration (Real). Nothing couples options 3 and 4 - real integration can run without dry run ever being done. Fix: collapse options 3+4 into a single "Integration" option that (1) runs the dry run and displays all output normally, (2) prints `--- DRY RUN COMPLETE ---`, (3) prompts `Proceed with real integration? [y/N]:` with default N and no timeout, (4) runs real integration only on explicit Y. This is a launch.bat change - can be done without the full Program.cs menu refactor (TIER 3). Constraint: direct `AudioManager.exe integrate` CLI still bypasses the gate - acceptable since David uses launch.bat exclusively. The safety property this enforces is not "dry run technically ran" but "you read the output before pressing Y" - the only way to confirm is to have your eyes on the terminal.

- [ ] **"(with X)" tag fixer** - Three tracks in the current batch use "(with X)" instead of "(feat. X)": `Nicki Minaj - Barbie World (with Aqua)`, `Perry Como - Catch a Falling Star (with Mitchell Ayres and His Orchestra & The Ray Charles Singers)`, `Perry Como - Magic Moments (with Mitchell Ayres and His Orchestra & The Ray Charles Singers)`. Current TagFixer strips `(feat. X)` / `(ft. X)` but silently ignores `(with X)` - the parenthetical stays in the title tag. Fix: add `\s*\(with\s+[^)]+\)` to `RemoveParentheticals` (title + album); add a matching capture pattern to `ExtractAndFixArtists` to pull collaborating artists into TPE1. Edge case: Perry Como's "with" credits are long orchestral names ("Mitchell Ayres and His Orchestra & The Ray Charles Singers"), not featured artists in the modern sense. Recommend extracting them into TPE1 anyway for consistency, but they will produce long artist strings - check post-dry-run whether they cause routing issues (new artist folders). If so, add them to `artist-name-overrides.xml` to preserve casing.

- [ ] **In-batch duplicate detection** - No guard exists for two copies of the same song landing in NewMusic. If it happens, TagFixer processes both, and MusicIntegrator routes both - the second would either collide with the first mid-batch or create a duplicate in the library that only shows up in the next LibChecker run. Fix: before routing begins, normalise artist+title across all NewMusic files, warn on any two sharing the same key as `[WARN: IN-BATCH DUPLICATE]` in dry-run output. Non-blocking - flag only, let the user delete before real run. Exact match only for now; fuzzy variant detection (`feat.` vs `with`, `(Live)` suffix) is TIER 4.

- [ ] **Album subfolder prep check (current batch)** - Batch contains two album subfolders: `Akon - BEAUTIFUL DAY/` (15 tracks, ALL CAPS filenames) and `Shaboozey - Where I've Been, Isn't Where I'm Going_ The Complete Edition/` (18 tracks). TagFixer uses `SearchOption.AllDirectories` so it will find these. Risk areas: (a) files downloaded as a batch may have missing or incomplete artist/album tags - if artist tag is empty, rename produces `- TITLE.mp3`; (b) album tag might be empty meaning all 15 Akon tracks route to Singles instead of an album subfolder; (c) Shaboozey album likely contains "A Bar Song (Tipsy)" which may already be in the library - standard duplicate-vs-library detection should catch it. **Mitigation: run dry run first and inspect the Akon + Shaboozey blocks carefully before real run.** No code change needed - dry run will surface any issues.
...
- [ ] **"(Single Version)" not stripped from title** - `RemoveParentheticals` handles `(Album Version)` and `(Version)` but not `(Single Version)`. Current batch: `Sade - Smooth Operator (Single Version)` would keep the suffix in its title tag. Fix: add `\s*\(Single\s+Version\)` to the RemoveParentheticals patterns. Also check for `(Radio Version)` - currently handled in `StripAlbumSuffixes` (album field only) but NOT in `RemoveParentheticals` (title field). Batch these two additions together - one small change, two patterns.
