...
- [ ] **[GUI] Acquire tab polish** - MVP shipped 2026-08-31 (see HISTORY.md), cheap/fast on purpose. Deferred: cache fetched tracks to disk (currently in-memory only, lost on tab rebuild/refresh); manual override for a verify-download false negative/positive (fuzzy match has no correction UI); progress feedback during Fetch/Sync (spinner only via label text, no per-track progress); handle a Liked Songs / inbox playlist larger than one API page gracefully in the GUI (backend already paginates, GUI has no test against a large real playlist yet); dedupe protection if Sync Liked is clicked twice before a page refresh.
  - **Table redesign (2026-09-01):** track table rebuilt as a full-width, larger-font checklist - one row per track with Artist/Title/Album/Year/Deemix-link/Manual-tickbox columns (`_build_sync_liked_card`/`track_table` in `gui/tabs/acquire.py`). Manual tickbox is in-memory only for now (lost on refresh, same limitation as the cached-tracks item above).
  ...
  - **State persistence across page reload (2026-09-01)** - fetched tracks and manual-tick state live only in the in-memory `_state` dict in `acquire.py`, keyed to nothing durable. A browser reload or navigating away and back to the Acquire tab currently loses the fetched rows and every tick David has made. Needs real state management: persist `_state["tracks"]` and `_state["manual"]` to disk (e.g. alongside `config.ACQUIRE_STATE_JSON`, which today only stores the last playlist ID) keyed by playlist ID, and reload it into the table on tab build instead of starting empty. Same underlying gap as the "cache fetched tracks to disk" item above - do them together.
