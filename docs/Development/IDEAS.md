...
**ACTIVE FOCUS:** Routing test suite complete (all tests green as of 2026-05-31). LibChecker.CheckCompilationsFolder tightened 2026-06-03 (genuine various-artist compilations now exempt). Schema enforcement complete 2026-06-06. Remaining TIER 1: AutoCommit hang bug (detected 2026-06-06).
...
- [ ] **Re-enable and improve album art dimensions check in LibChecker** - `CheckAlbumCoverDimensions` was temporarily commented out (2026-06-24) to unblock integration while the existing library has ~150+ low-res tracks. Re-enable once album art remediation is done (see TIER 4 "Album art remediation" entry). Also consider: suppress the check for tracks that have no cover at all (already caught by `CheckAlbumCoverCount`), and print a single summary line instead of one line per track. The full list floods terminal output and is better suited to the report file.
