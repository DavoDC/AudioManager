...
- [ ] **Sort routing output by artist/album grouping** - Current dry-run output is ordered by file-processing sequence, so tracks from the same artist/destination are interspersed. Example: Goggins track, then Foster Wallace track, then another Goggins track. Sort output by: (1) routing destination path, then (2) artist, then (3) album, then (4) title. Grouped output makes the dry-run scannable and catches routing anomalies by visual clustering.

- [ ] **Routing reason completeness for Misc** - "No artist folder found in library" is the outcome, not the reasoning. The full decision chain should be explicit: how many tracks for this artist in the batch, how many in the existing library (including Misc), what the threshold is, and why Misc was chosen. Example: "no artist folder; 2 tracks in batch + 0 in library = 2 total, below threshold 3 -> Misc". Makes dry-run auditable without re-running the analysis.
...
- [ ] **Left-align `[DRY RUN]` output lines** - `[DRY RUN] Would delete/keep` lines have leading spaces, making them harder to scan. Strip leading whitespace so they align flush-left with `[AUTO]` routing lines.
