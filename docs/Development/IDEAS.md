...
- [ ] **DRY RUN: Show simulated (post-TagFixer) tags in routing blocks - dry-run must be as close to real integration as possible** - Dry-run routing blocks currently show original raw tags, so the user must mentally cross-reference two separate outputs (TagFixer section + routing section) to validate a file. Goal: dry-run should be a faithful preview of real integration. Routing blocks must show the tags AS THEY WILL BE after TagFixer runs - cleaned artist name (no "Scott adams", no "feat." in title), cleaned album, correct genre. If the routing block shows "feat. Artist" in the title, that is wrong and misleading - the real integration will have that stripped before routing. **Fix approach:** Build a lightweight tag-simulation layer: before routing each file in dry-run, apply TagFixer's in-memory transformations (RemoveParentheticals, ExtractAndFixArtists, DetermineGenre) to produce a simulated Track, then use that for both the routing decision AND the display. TCMP can be excluded (user trusts it). The result: one unified review pass where you can confirm "tags look right AND routing looks right" without switching mental context. This is the single most important dry-run improvement - everything else is cosmetic.
