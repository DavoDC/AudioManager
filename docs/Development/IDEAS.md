...
- [ ] **LibChecker filename check doesn't validate artist semicolons** - `CheckFilename()` checks each artist is a substring of the filename, but doesn't verify the semicolon delimiter between artists is present. Result: "T.I.Cee Lo Green - Hello.mp3" passes (contains both "T.I." and "Cee Lo Green") even though "T.I.;Cee Lo Green - Hello.mp3" is the correct format. At least 2 library files affected: `Artists/T.I/Singles/T.I.Cee Lo Green - Hello.mp3` and `T.I.Dr. Dre - Popped Off.mp3`. Fix: after checking each artist substring, verify the filename contains either `{primary artist};` (for multi-artist tracks) or `{primary artist} -` (for single-artist). Also rename the 2 existing malformed files via Mp3tag.
