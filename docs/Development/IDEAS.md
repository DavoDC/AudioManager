...
- [ ] **CRITICAL: Duplicate handling with informed user decision** - When integrator finds a duplicate (same artist+title already in library), display BOTH versions side-by-side so user can decide: (1) **Show new music file:** full path + metadata (artist, title, album, year, duration, genres) from MP3 tags. (2) **Show existing file:** full path in library + metadata from AudioMirror XML. (3) **Offer 3 options:** [D] Delete from NewMusic, [L] Delete from Library (replace old with new per album-preference rule), [K] Keep both, [Q] Quit. **Why critical:** Dry-run revealed many duplicates (e.g., Akira The Don singles being replaced by album versions). Rule 1 from STAGE_3B: "If singles exist in library + full album version is available, DELETE singles and KEEP ALBUM." User needs full context to make correct choice. Currently offers only [D], [K], [Q] with no [L] option.

- [ ] **HIGH PRIORITY: Auto-delete Akira The Don instrumentals** - Integrate into TagFixer: any track where artist contains "Akira The Don" AND (title contains "Instrumental" OR title ends with "(Instrumental)") should be automatically deleted from NewMusic before integration. Use **artist+title matching** (from ID3 tags), not filename matching. Rule 2 from STAGE_3B: "Auto-delete any track matching: Artist = Akira The Don AND filename contains Instrumental". Dry-run showed multiple "Akira The Don - [Album] (Instrumental).mp3" files that should be silently removed. **Why:** instrumentals are not wanted in the collection; auto-delete prevents them from entering the library at all.

- [ ] **HIGH PRIORITY: Improve TagFixer output readability** - Current dry-run output is hard to interpret (uses unclear `␦` symbol, scatters one file's changes across multiple lines, filename changes not explicitly labeled). Reformat to show per-file changes grouped and clearly labeled. **Format:** Group all tag changes for a single file together under a header `[WOULD FIX] Filename`, then list each change with clear arrow notation `OldValue -> NewValue`, and explicitly label filename changes as `Filename: OldName -> NewName`. **Why critical:** User must be able to validate tag fixes before real integration. Current format with `␦` symbol and scattered output is unreadable and prevents confident dry-run review. Dry-run validation depends on clear visibility. **Implementation:** Modify TagFixer.cs output section to: (1) Replace `␦` with `->`, (2) Group per-file changes instead of logging per-tag, (3) Explicitly label filename renames, (4) Consider collapsing the quick-scan top section or removing redundancy.
