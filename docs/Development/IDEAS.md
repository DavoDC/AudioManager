...
- [ ] **[BLOCKING][RETURNING] Batch album count missing for existing artist folders** - When multiple songs from the same album are integrated into an EXISTING artist folder (e.g., 5 Jay-Z songs from "The Blueprint"), `CountAlbumSongs` only checks the library - not other songs in the current batch. Each song independently sees "0 songs from album" and routes to Singles even though 5+ batch songs should trigger an album subfolder. The scan-ahead fix (a6fba7a7) handled NEW artist creation only - did not fix existing artist album routing. **Fix:** when computing album song count for routing, add batch songs (same album, same primary artist) to the library count. Add test: `Routing_ExistingArtist_MultipleBatchSongsFromSameAlbum_RoutesToAlbumFolder`. **Note:** also affects Misc Migration - migrated songs routed to Singles without checking if batch songs put them over the album threshold (e.g., "Dream Chasin'" migrated to Chiddy Bang/Singles but should check Breakfast album count).
...
- [ ] **Fix "only 0 songs from album" wording in routing reason strings** - "only 0 songs from album -> Singles/" reads awkwardly. Should be "no songs from album -> Singles/". Audit ALL dynamic routing reason strings for similar weird wording before fixing.
