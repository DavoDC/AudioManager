...
- [ ] **NewMusic Decision Log - capture routing decisions during review** - As music is reviewed (Stage 3B), every keep/delete/reorganize decision contains routing logic that's invisible to the integrator. Log each decision with: artist, album, track name, action (keep/delete/reorganize), reason, destination folder. Store in structured format (JSON or CSV appended to workflow doc). Integrator can later reference this corpus to understand placement rules and improve routing heuristics. Extract 3+ patterns from decisions to propose as rules (e.g. "Akira instrumentals always delete", "album versions preferred over singles", "Motivation genre for specific artists"). **Success:** All STAGE 3B decisions logged; at least 3 actionable patterns extracted. Start this session with current NewMusic batch.
