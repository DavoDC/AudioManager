...
- [ ] **Artist override config validation - warn on missing overrides** - The artist-name-overrides.xml system preserves casing for configured artists (e.g. "mike." stays lowercase). However, there's a risk of the rule being accidentally lost if an override is removed from config without being added back later. Example: code comments reference "Scott adams" needing title-case fix, but if "Scott Adams" ever needed non-standard casing, the override could be forgotten. Add a validation/warning system: (1) When a tag file contains an artist with unusual casing (detected by comparing the parsed tag against title-cased version), check if that artist is in the overrides config. (2) If not, warn: "Found artist with non-standard casing not in overrides.xml: 'scott.' - consider adding to config." This surfaces missing overrides before they're silently lost.
