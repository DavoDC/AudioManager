...
- [ ] **NewMusic Decision Log - capture integration & routing decisions during import** - As new music is reviewed (Stage 3B), every keep/delete/reorganize decision reveals routing logic and folder-placement rules that are invisible to the integrator. Log each decision with: artist, album, track name, action (keep/delete/reorganize), reason, destination folder in Audio library. Store in structured format (JSON or CSV in workflow doc). Integrator can reference this corpus to: (1) understand why certain tracks go to certain folders, (2) discover patterns for auto-routing improvements, (3) detect hard-to-classify cases. Extract 3+ actionable patterns from decisions (e.g. "Akira The Don instrumentals always delete before integration", "prefer album versions over singles when both exist", "Motivation genre for specific artists → Musivation folder"). **Distinct from TIER 4 "Review mode"** (which logs why existing library songs are kept/removed). This is about FORWARD routing decisions for new imports. **Success:** All STAGE 3B decisions logged; at least 3 routing patterns extracted. Start this session with current NewMusic batch.
