...
- [ ] **Claude runs user dry-run path during development** - HIGH PRIORITY - When fixing bugs or implementing features, Claude executes the exact same menu path the user takes: build → select "Dry Run (NewMusic Batch)" → review output. Claude runs this with test data to verify fixes in real time, catching routing/tagging issues before user integration. Equivalent to user manual dry-run, but safe (no file moves) so Claude can run as standard development practice. Behavior change: Claude no longer simulates or describes behavior - Claude actually runs the tool, sees output, validates. Payback: catches edge cases immediately; much higher confidence than code inspection alone. Optional integration: if dry-run output is captured, can feed into automated test validation (but the core value is real execution during development).

- [ ] **Dry-run output quality pass** - Three display improvements in the same rendering code - tackle as one session: (1) **Sort by destination** - current output is ordered by file-processing sequence, so tracks from the same artist/destination are interspersed. Sort output by: routing destination path -> artist -> album -> title. Grouped output makes the dry-run scannable and catches routing anomalies by visual clustering. (2) **Routing reason completeness** - "No artist folder found in library" is the outcome, not the reasoning. State the full decision chain: how many tracks for this artist in the batch, how many in the existing library (including Misc), what the threshold is, and why Misc was chosen. Example: "no artist folder; 2 tracks in batch + 0 in library = 2 total, below threshold 3 -> Misc". (3) **Left-align `[DRY RUN]` lines** - strip leading whitespace so they align flush-left with `[AUTO]` routing lines.
