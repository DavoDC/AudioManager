...
- [ ] **CRITICAL: Audit and restore lost Scott Adams casing rule** - During the 2026-05-05 refactor to add artist override config system, the existing Scott Adams title-casing rule was accidentally lost/not ported to the new config. Scott Adams must be title-cased (not lowercase). This is a real library rule that was silently dropped. Add to artist-name-overrides.xml and verify any other existing rules weren't similarly lost. See HISTORY.md (2026-05-05) for context on the refactor that caused this oversight.

- [ ] **Audit: move all critical artist casing rules to config** - The artist-name-overrides.xml system is durable and version-controlled. However, critical casing rules can be scattered in code comments and easily forgotten/lost during refactors. Audit the codebase (TagFixer.cs comments, CLAUDE.md notes, HISTORY.md, code history) for any other artist casing rules and move them all to artist-name-overrides.xml so the rule lives in config, not comments. This prevents "accidentally blatted" rules.
