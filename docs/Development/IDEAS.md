...
- [ ] **Album art dimensions - Phase 2 (Analyse) + Phase 3 (Enforce)** - Phase 1 (Capture) done 2026-05-31. Format updated 2026-06-02: cover fields now grouped under `<AlbumCover><Count>`, `<Width>`, `<Height>` (force regen required to migrate existing XMLs). Next phases:
...
- [ ] **Thin verify.bat - move test aggregation into exe as `--verify` flag** - `scripts/dev/verify.bat` runs `--test` and `--routing-manifest` in two separate subprocess calls, captures output to temp files, then does bat-level string parsing (`for /f tokens=2,4 findstr "^Results:"`) and arithmetic (`set /a TOTAL_PASS=...`) to combine pass/fail counts into a summary line. This is logic the exe should own. A `--verify` flag in Program.cs would run unit tests then manifest tests internally, print a combined "Results: X passed, Y failed" line, and exit with the right code. verify.bat drops from ~60 lines to ~15 (same shape as test.bat: build, run `--verify`, check exit). Fits the thin-bat principle from Phase 1/2.
