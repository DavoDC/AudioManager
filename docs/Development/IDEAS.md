...
- [ ] **Graceful handling of missing/incomplete tags in album subfolders** - Tracks downloaded as albums sometimes have incomplete metadata (missing artist, missing album tag, or numbered-only filenames like `01 - TITLE.mp3`). Current behavior when artist tag is empty: rename produces `- TITLE.mp3` (broken filename); missing album tag: all tracks route to Singles regardless of count. Fix: (a) in TagFixer rename step, guard against empty artist/title producing malformed filenames - fall back to original filename and log a warning; (b) in MusicIntegrator routing, if album tag is empty on a file from a subfolder, inherit album name from the folder name as a fallback. Not blocking - tracks with good tags (the normal case) are unaffected.
