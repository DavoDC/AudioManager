...
- [x] **Add `libchecker-exceptions.example.xml`** (COMPLETED 2026-04-27) - Added example template showing exception format and matching conditions. Real exceptions go in libchecker-exceptions.xml (gitignored).
- [x] **Split `scripts/` into `scripts/` and `scripts/once_off/`** (ALREADY DONE) - One-off Python scripts already in scripts/once-off/; production launchers (launch.bat, build.bat) in scripts/ root.
- [x] **Dry-run covers tag changes and renames** (COMPLETED 2026-04-27) - TagFixer now implements tag changes and renames as separate pre-integration step. `tagfix --dry-run` shows all changes without modifying files. Integrator dry-run shows routing decisions. Both covered.
- [x] **Generate reports in markdown (.md) instead of .txt** (COMPLETED 2026-04-27) - ReportWriter and MusicIntegrator now output .md files with markdown syntax: headers (#), bold (**), bullet points, inline code. Better readability in GitHub and editors.
...
- [x] **README: mention folder picker** (COMPLETED 2026-04-27) - Added to Integration Pipeline section: "Optional folder picker for ambiguous routes (Films/Shows/Anime media, Compilations)"
- [x] **README: list LibChecker checks** (COMPLETED 2026-04-27) - Added new "LibChecker Validations" section listing all 11 checks performed.
