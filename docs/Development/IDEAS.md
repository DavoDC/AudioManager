...
- [ ] **UX: Detect compilation vs artist album for smarter [L] recommendation** - Current logic only recommends [L] when `relMirrorPath` starts with `Compilations\` (root folder only). Misses: library track in a named compilation series that lives elsewhere (e.g. `Musivation\Akira The Don\MEANINGWAVE MASTERPIECES V\`). Rule: Artist album > Compilation > Single. Detect compilations by: (1) checking `Compilations\` root folder (existing), (2) checking album name for keywords like "Best Of", "Greatest Hits", "Collection", "Anthology", "Masterpieces", or volume indicators (Vol., V, IV, III, II). If library album matches a compilation keyword and new file has a named artist album, recommend [L] with reason.
