...
- [ ] **CRITICAL BUG: Album subfolder logic not implemented** - Dry-run shows tracks routed to album folders even when only 1 song from that album. Rule: **2+ songs from same album in NewMusic batch → album subfolder; 1 song → Singles/**. Examples: "Evergreen Nights" (Dylan Owen) shown routed to `Artists\Dylan Owen\Evergreen Nights\` but if only 1-2 tracks from that album in the batch, should go to `Artists\Dylan Owen\Singles\`. Root cause: GetDestDir() counts artists for scan-ahead but doesn't count per-album. Must fix album-counting logic.

- [ ] **CRITICAL BUG: Wrong routing for Akira The Don - routes to Artists instead of Musivation** - Dry-run 2026-04-28 shows UNSTOPPABLE routed to `Artists\Akira The Don\UNSTOPPABLE\` but should route to `Musivation\Akira The Don\People\Brian Tracy\UNSTOPPABLE\`. Root cause: GetDestDir() doesn't recognize Akira The Don as a special artist requiring Musivation folder + People subfolder for collaborations. Must also apply album subfolder rule above (2+ songs from album → album folder).

- [ ] **BUG: Filename not regenerated after tag fixing** - Dry-run shows filename with old featured artist parenthetical (e.g., "Loophole (feat. Nowdaze).mp3") even though TagFixer cleaned the Title tag to just "Loophole". The filename should be regenerated from the NEW tag values, not from the original filename. When displaying routing decisions, show the FINAL filename that would result, not intermediate state. This affects consistency and makes dry-run harder to trust.
