...
- [x] **CRITICAL: Duplicate handling with informed user decision** - When integrator finds a duplicate (same artist+title already in library), display BOTH versions with full paths so user can decide: [D] Delete from NewMusic, [L] Delete from Library (replace old with new per album-preference rule), [K] Keep both, [Q] Quit. Implemented in MusicIntegrator (commit f9f96a79): added [L] option that deletes from library file, DeriveLibraryPathFromMirrorPath() helper to map AudioMirror XML paths to actual Audio library paths. Dry-run shows preview, real run executes deletion. **DONE (2026-04-28).**

- [x] **HIGH PRIORITY: Auto-delete Akira The Don instrumentals** - Integrated into TagFixer: any track where artist contains "Akira The Don" AND title contains/ends with "Instrumental" is automatically deleted before tag fixing. Uses artist+title matching from ID3 tags (not filename). Implemented in TagFixer.cs (commit 020eb8d): added auto-delete check after tag read, logs [AUTO-DELETED] in real run and [WOULD AUTO-DELETE] in dry-run. Silently removes instrumentals before they enter the library. **DONE (2026-04-28).**
