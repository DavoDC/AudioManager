...
- [ ] **[BLOCKING] Post-integration validation reports ghost-XML false positives** - Post-integration validation runs incremental regen (`Recreated: False`), which adds XMLs for new files but never removes XMLs for files the integration just deleted (`[L]` duplicate decisions) or moved (Misc migration). The validated mirror therefore describes a library that no longer exists, and LibChecker flags the ghosts as duplicates / misplaced songs. **Proof (2026-06-28 run):** post-integration regen reported `MP3 file count: 5693` but `Tags parsed: 5702` - a 9-XML delta exactly equal to the 9 library locations vacated that run (8 `[L]` deletes + 1 Misc move). 11 of 12 LibChecker hits were ghosts; only the B.I.G. album-suffix hit (below) was real. **Fix (smallest):** post-integration validation must force-regen before LibChecker. **Fix (deeper, preferred):** make incremental Reflector prune orphaned XMLs (delete any XML whose MP3 is gone) - this also retires the ParseCache "deleted MP3 persists until force regen" caveat and the "force regen required after deletion" footnotes. Guard: only prune when the Audio root enumerated successfully. Full analysis + proof table: `docs/References/Post-Integration-Validation.md`.

- [ ] **[BLOCKING] Dry-run projected LibChecker - validate the final state before moving files** - The dry run already computes every fixed tag, every routing destination, and every library delete. Build the projected post-integration mirror in memory (`current parsed tags` - `deleted/moved-from TrackTags` + `synthesized destination TrackTags with fixed tags`) and run LibChecker on it. Predicts the real post-integration state with zero ghost false positives (deletions are modelled correctly), BEFORE any irreversible move. No MP3 reads needed - LibChecker consumes `List<TrackTag>`, and the integrator already holds the fixed tags. This makes "a clean integration leaves a clean library" achievable and enforceable: print a projected-LibChecker section in the dry run, and have integration refuse to proceed (or require an explicit override) when the projection is not clean. Would have caught the B.I.G. issue on 2026-06-28 while files were still staged. This is the primary validation; post-integration validation becomes a final confirmation. See `docs/References/Post-Integration-Validation.md`.

- [ ] **[BLOCKING] Album version-suffix normalization gap splits albums** - `The Notorious B.I.G. - I Love The Dough (2014 Remaster)` (album `Life After Death (2014 Remastered Edition)`) routed to a `Life After Death (2014 Remastered Edition)/` subfolder distinct from the library's existing `Life After Death/` tracks - splitting the album and tripping the LibChecker album-subfolder rule. Two compounding defects: (1) TagFixer strips `(Explicit Version)`, `(Expanded Edition)`, `(feat. ...)` but not `(... Remaster/Remastered/Anniversary/Mono/Stereo ...)` - the strip list is ad hoc and incomplete; (2) scan-ahead's album-count matched the album loosely ("4 songs") while folder placement used the exact suffixed name (file landed alone) - a routing-vs-placement normalization divergence. **Fix:** one canonical album-version normalizer used by BOTH the album-count logic and the destination-path builder, so count and placement can never disagree. Decide a consistent Deluxe-vs-base merge rule while here (BULLY-DELUXE is currently kept separate by design). See `docs/References/Post-Integration-Validation.md`.

- [ ] **[INVESTIGATE -> likely TIER 1] Multi-artist destination filename keeps "; " while tag normalizes to ";"** - In the 2026-06-28 confidence report every multi-artist destination filename shows the `"; "` (space) form (e.g. `Akira The Don; David Lynch - KALI YUGA.mp3`) while the artist tag is normalized to `";"` (`Akira The Don;David Lynch`). LibChecker's filename-vs-tag check did not flag it. Either the on-disk filename is correct and only the log display is stale, OR TagFixer normalizes the tag separator but the file rename uses the un-normalized artist string (real tag/filename mismatch + a gap in `CheckFilenamesCasingsMatchArtistTags`, which may compare casing but not separator spacing). **First step:** read one multi-artist file's on-disk name vs its `<Artists>` tag. If mismatched, this is two defects (TagFixer rename + LibChecker check). Do not fix before confirming. See `docs/References/Post-Integration-Validation.md`.

- [ ] **[BLOCKING] AudioMirrorCommitter excludes LastRunInfo.txt from auto-commit** - `TryCommit()` stages only `AUDIO_MIRROR/` (line 83 of `AudioMirrorCommitter.cs`: `RunGit(repoPath, "add AUDIO_MIRROR/")`), but `LastRunInfo.txt` lives at the AudioMirror repo root (`Constants.LastRunInfoFilePath = MirrorRepoPath + "LastRunInfo.txt"`). Every auto-commit silently omits it. CLAUDE.md rule: "AudioMirror audit metadata (LastRunInfo.txt) is as much part of the artifact as the data itself - version-control it." Confirmed missing: the 2026-06-28 "Jun 28 Update" auto-commit did not include `LastRunInfo.txt`. **Fix (one line):** change `RunGit(repoPath, "add AUDIO_MIRROR/")` to `RunGit(repoPath, "add AUDIO_MIRROR/ LastRunInfo.txt")`. Add a test to `AudioMirrorCommitterTests.cs` that verifies `LastRunInfo.txt` is staged alongside `AUDIO_MIRROR/` in a real commit call.

- [ ] **[BLOCKING] Integration completes but subsequent LibChecker run uses incremental regen - shows ghost false positives** - After a real integration (files deleted via [L] decisions, files moved via Misc migration), the XMLs for removed library locations persist because incremental Reflector only creates/updates XMLs - it never prunes. Running analysis (incremental) immediately after integration reports many LibChecker issues that are all ghosts; only real issues survive a force-regen. **Confirmed 2026-06-28:** incremental post-integration analysis showed a large number of LibChecker hits; `--force-regen` reduced it to exactly 1 real issue (the B.I.G. album-suffix case). **Fix (immediate workaround):** after every real integration, always run `analysis --force-regen` before trusting LibChecker output - never rely on incremental. **Fix (correct):** integration completion should auto-trigger a force-regen pass before running LibChecker and before calling `TryCommit` - so the auto-committed mirror is always prune-clean. See also TIER 1 item "Post-integration validation reports ghost-XML false positives" for the deeper orphan-pruning fix.
...
- [ ] **Dry-run L-decision files: routing destination not shown** - When a duplicate is auto-resolved with [L] (keep new file, delete library copy), dry-run sets `SkipRouting=true` to avoid double output in the duplicate section. Side effect: the routing destination for that file is never printed in the dry-run routing block. Users don't see where e.g. `KALI YUGA` will land after replacing the library copy. Fix: in the duplicate section output, print the anticipated routing destination alongside the "Would keep new file" line.

- [ ] **Misc migration WARN message misleading for dup-resolved files** - When duplicate resolution auto-accepts [L] (delete library copy), the library file is physically deleted during real integration before `RunMiscMigration()` runs. If the deleted file was also a Misc migration candidate (e.g. `Chiddy Bang;Icona Pop - Mind Your Manners.mp3`), `RunMiscMigration` prints `[WARN] Not found in library (stale mirror?)` instead of a clearer message. Behavior is safe (skips the file), but the message is confusing. Fix: flag migration candidates that were also [L]-decided duplicates and print `[INFO] Already deleted by duplicate resolution: {filename}` instead of the stale-mirror warning.
