...
- [ ] **DRY RUN: Show simulated tag values in routing blocks so tag + routing can be reviewed together** - Currently dry-run routing blocks show original (unmodified) tags, so the user must cross-reference the separate TagFixer output to verify tags are correct. Goal: routing decision blocks should show the tags AS THEY WILL BE after TagFixer runs (simulated), so the user can confirm both "is routing correct?" and "are tags correct?" in one glance, without needing to read separate TagFixer output. TCMP changes can be excluded (user trusts those). **Fix approach:** Apply tag modifications in-memory as files are read (simulate what TagFixer would do, excluding TCMP), so routing blocks display cleaned artist names, genres, filenames. Could use temporary Track objects with simulated tag values. **Why elevated:** User confirmed during real dry-run that tag issues are visible in routing blocks - combining the review eliminates a manual cross-check step.
