...
`CLAUDE_CODE_DISABLE_1M_CONTEXT=1` is currently set globally (see `feedback_1m_context_resume_sticky.md` in the workspace) - this blocks the `[1m]` model variants from even appearing in the picker. This build's "go deep, don't run out of context mid-build" goal may want 1M context given the scope (six tabs, full chart library, deep research folded in). But 1M context is never free - it draws usage credits on every plan, Fable's 1M rate likely draws faster than Sonnet's.

**Decision needed before starting, not discovered mid-session:** if David wants 1M context available for this Fable session, temporarily unset `CLAUDE_CODE_DISABLE_1M_CONTEXT` in both `~/.claude/settings.json` and the workspace `.claude/settings.json`, select `fable[1m]` explicitly via `/model`, then re-set the env var back to `1` afterward so it doesn't stick across future resumed sessions (the whole reason that var exists - see the feedback file for the incident that caused it). Don't leave it unset "just in case."
