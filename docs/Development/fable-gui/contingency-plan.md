...
**Decision made (2026-07-02): enable 1M context for this build.** Before starting the Fable session, temporarily unset `CLAUDE_CODE_DISABLE_1M_CONTEXT` in both `~/.claude/settings.json` and the workspace `.claude/settings.json`, select `fable[1m]` explicitly via `/model`. **Re-set the env var back to `1` immediately after this session ends** so it doesn't stick across future resumed sessions (the whole reason that var exists - see `feedback_1m_context_resume_sticky.md` for the incident that caused it). Don't leave it unset "just in case" once this build wraps up.
