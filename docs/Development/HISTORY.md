...
Fix: a new pure function `_is_custom_rule_warning_line()` detects both warning formats by the shared substring `[WARN] Custom tag rule` (capitalised, singular "rule"), which deliberately excludes the unrelated load-time warning `[WARN] Could not load custom tag rules (...)` (lowercase "custom", plural "rules") - a config-file problem, not a rule-authoring one, and out of scope here. `_render_console_lines()` now tags matching lines with a `custom-rule-warning-line` CSS class, styled in `gui/theme.py` with the existing warning-toned `--accent3` amber (the same color already used for `.gap-note`, `.simulate-banner`, and `.status-badge.draft`) rather than inventing a new color. A new `count_custom_rule_warnings()` helper feeds a visible pill badge ("N rule warnings - open the dry-run output below to see them") rendered above the "Dry-run output" expansion itself, so a bad rule is noticeable without opening the collapsed section at all. Six new tests in `gui/tests/test_tagfix.py` cover both warning line formats (including the no-quoted-id "missing id attribute" variant), confirm ordinary change lines and the unrelated load-time warning are excluded, and check the count helper and the rendered warning class.
...
Fix, entirely inside `gui/tabs/tagfix.py`'s `_open_rule_dialog`: a new pure function `literal_value_regex_metachars(action, pattern, value)` returns which of the regex metacharacters (`. ^ $ * + ? ( ) [ ] { } | \`) appear in Value, in first-seen order, whenever `action == "regex-replace"` and `pattern` is genuinely empty - matching the C# `IsNullOrEmpty` check exactly, so a whitespace-only Pattern does not trigger it either. The dialog wires this to `on_change` on the Value and Pattern inputs and the Action select (the same `on_change`/`e.value` idiom already used elsewhere in the tab, e.g. `gui/tabs/library.py`'s search box), redrawing an inline warning label in a dedicated `regex_warning_box` column as the user types - no Save click required. The warning is deliberately not routed through `error_box` (the `validate_rule()`-driven, Save-blocking errors) so it reads as informational rather than as something preventing Save; it never appears when Pattern is set or the action is `set-value`, and lists only the characters actually present (e.g. Value `"(Official Video)"` warns about `(` and `)` only). Six new tests in `gui/tests/test_tagfix.py` cover the function directly: no warning with a non-empty Pattern, no warning for `set-value`, no warning when Value has no metacharacters, only-the-present-characters-in-order for `"(Official Video)"` and `"a.b.c*d"`, and the whitespace-only-Pattern-does-not-count-as-empty edge case matching the C# side.

---

## 2026-09-05 - Tag Fix's two rule writers now merge by id into a freshly re-read file instead of overwriting from a stale snapshot

Closed the "Two rule writers read-modify-write the rules file from independent snapshots" item from the 2026-09-05 "Library Intake design/usability review" section of IDEAS.md. `_toggle_enabled()` and the rule-edit dialog's `do_save()` in `gui/tabs/tagfix.py` each call `rules_store.load_rules()`, mutate their own in-memory copy, and `save_rules()` the whole list back - the concern was a rule-edit dialog left open while a switch is toggled elsewhere (or vice versa) letting the second writer's `save_rules()` overwrite the whole file from a stale snapshot, silently reverting whichever change happened first. On inspection, both writers already re-read `rules_store.load_rules()` synchronously at the moment their button is clicked rather than at dialog-open time, which already closes most of the window; NiceGUI's synchronous (non-async) event handlers also run to completion without yielding, so two clicks can't interleave within a single process either.

To make that guarantee explicit and testable rather than relying on nobody moving the `load_rules()` call earlier in a future edit, the merge-by-id step in each writer was pulled out into small pure functions: `merge_toggle_enabled(rules, rule_id, enabled)` flips one rule's flag inside an already-freshly-loaded list, and `merge_saved_rule(existing_rules, new_rule, editing_id)` replaces (when editing) or appends (when adding) one rule inside an already-freshly-loaded list. `do_save()` was also changed to re-`load_rules()` a second time immediately before this merge-and-save step, rather than reusing the list it loaded earlier for `validate_rule()`'s uniqueness check - closing the one real (if narrow) gap between that validation read and the eventual save. `gui/rules_store.py` was not touched: no locking, versioning, or file-format change, per the "keep it small" scope of this item. Two new tests in `gui/tests/test_tagfix.py` exercise the full scenario end to end against real XML round trips - toggling a rule after an independent add landed on disk in between, and saving an edited rule after an independent toggle landed on disk in between - plus direct unit tests of both merge helpers.

---

## 2026-09-05 - Acquire's manual override is now a true toggle, not one-way

Closed the "Manual override is one-way - a mis-click can only be undone by Clear, which wipes everything" item from the 2026-09-05 "Library Intake design/usability review" section of IDEAS.md. `toggle_manual_override()` (`gui/tabs/acquire.py`) unconditionally set `_state["manual_override"][row_key] = True` on every click despite its name - there was no path that ever removed a key. Once a row was overridden, `_run_check_against_downloads()` skipped it forever, so a single mis-click in a 200-row fetched table could only be undone with the Clear button, which discards the whole fetched playlist and every other override along with it.

Fix: `toggle_manual_override()` now checks whether `row_key` is already present in `_state["manual_override"]` - if so, it deletes the key (genuinely absent, not present-and-False) instead of re-setting it; only when the key is absent does it flip the Downloaded value and set the override, as before. Persistence via `_save_tracks_cache()` runs after either branch, so the removal survives a reload exactly like the addition already did. No change was needed in `_run_check_against_downloads()`: it already treats "row_key not in overrides" as "let the real fuzzy-match scan decide", so un-overriding a row makes it eligible for the real scan again on the very next check with no extra logic. Three new tests in `gui/tests/test_acquire.py`: `test_toggle_manual_override_second_click_un_overrides` (second click removes the key), `test_toggle_manual_override_un_override_restores_scan_eligibility` (a re-check after un-override lets the real scan decide the row), and `test_toggle_manual_override_persists_the_removal_across_a_reload` (the deletion round-trips through `ACQUIRE_STATE_JSON`). The pre-existing `test_toggle_manual_override_flips_value_and_marks_the_row` was trimmed to cover only the first click, since its old second-click assertion encoded the one-way bug being fixed here.

---

## 2026-09-05 - Acquire's persisted row keys are now content-based, not fetch-order index

Closed the "Acquire's persisted row keys are index-based, so re-fetching a reordered playlist re-attaches ticks and manual overrides to the wrong tracks" item from the 2026-09-05 "Library Intake design/usability review" section of IDEAS.md. `row_key` was built as `f"{i}:{artist}:{title}"`, where `i` was the track's position in the fetched list, and both the Downloaded ticks and manual overrides were persisted to `ACQUIRE_STATE_JSON` under that key. Concrete failure: David marks a track as manually downloaded, later the Spotify playlist gets reordered (a song added upstream, a shuffle, a removal), and every index below the change point shifts by one or more - so on the next fetch the same numeric key now belongs to a different track, and the override silently pins the wrong song as downloaded while the track that actually had the override reverts to a fresh fuzzy match.

Fix: added `_content_key()` and `_row_keys_for_tracks()` in `gui/tabs/acquire.py`, which build the key from normalised artist+title alone - reusing the exact `clean_artist`/`clean_title`/`normalise` pipeline `_artist_title_match()` already uses, rather than inventing a second normalisation scheme. The common case (no duplicate artist/title pairs in a fetch) now produces a key that is completely independent of fetch order; a genuine duplicate (two tracks that normalise to the same content key) gets an occurrence-index suffix appended, but only once the collision is actually detected. `simulate()`, `_run_check_against_downloads()`, and `build_track_rows()` were switched from the old index-based f-string to this helper - `toggle_manual_override()` needed no change, since it only ever receives a row_key as a parameter. Regression test `test_manual_override_follows_the_track_after_a_reordered_refetch` in `gui/tests/test_acquire.py` sets a manual override, then reorders `_state["tracks"]` so a new track lands at the top (shifting every later index), and asserts the override still applies to the correct track by content; two more tests (`test_row_keys_are_independent_of_fetch_order`, `test_row_keys_disambiguate_genuine_duplicates`) cover the helper directly. Every pre-existing test that hardcoded an old index-based row_key string was updated to the new normalised format.

---

## 2026-09-05 - A skipped file no longer counts as moved in the Integration result panel

Closed the "A file the exe SKIPPED is reported as moved, and the 'N moved' summary can contradict the confidence strip directly below it" item from the 2026-09-05 "Library Intake design/usability review" section of IDEAS.md. `_update_exec_status()` (`gui/tabs/integration.py`) treated `[AUTO]` (moved) and `[SKIP]` (left in place - already exists at destination) as the same done state, and on a successful run `_finish_execute()` swept every remaining `queued`/`moving` row to `"done"` and counted every `"done"` row as moved. Concrete failure: a batch where 3 of 12 tracks already existed at their destination reported "Integration complete - 12 moved" with all 12 rows shown green, while `confidence_report_strip()` immediately below rendered the exe's own authoritative `Files in NewMusic: 12 | Moved: 9 | Skipped: 3` from the same run - two contradictory numbers on the same panel, no way to tell which files were still sitting in NewMusic.

Fix, in two parts: (1) `[SKIP]` lines now settle to a distinct `"skipped"` status in `_update_exec_status()`, rendered with its own label and CSS class (`.st-skipped`, `gui/theme.py`) instead of being lumped into `"done"`; the progress-bar completion tally was updated to count `"skipped"` as settled too. (2) `parse_confidence_report()` (`gui/routing.py`) now also parses the `Moved`/`Skipped`/`Files in NewMusic` numbers out of the count line into `moved_count`/`skipped_count`/`total_count`, and `_finish_execute()` prefers those parsed counts over its own locally-swept tally whenever the confidence report parsed successfully, falling back to the local count only when it didn't (e.g. dry runs, which never reach the confidence-report step). The summary line now reads e.g. "Integration complete - 9 moved, 3 already in library" and can no longer disagree with the strip beneath it. Four new tests: `test_update_exec_status_marks_skip_line_skipped_not_done`, `test_finish_execute_prefers_confidence_report_counts_over_local_sweep`, and `test_finish_execute_falls_back_to_local_counts_when_confidence_report_absent` in `gui/tests/test_integration.py`, plus `test_parse_confidence_report_parses_moved_and_skipped_counts_separately` and `test_parse_confidence_report_counts_are_none_when_count_line_missing` in `gui/tests/test_routing.py`.

Left explicitly untouched: the related but separate exec-status substring-guard bug noted in the same IDEAS.md review section (a short target's "artist - title" text matching as a substring of another target's, which can strand a row on "queued") - it lives in the same function but is a different bug with its own fix, out of scope here.
...
- **DECISION GATE: Python Rewrite vs .NET 8 Migration** - **Status: Resolved 2026-09-06, stay polyglot.** A fresh first-principles architecture review confirmed the open factor ("confirm Python libs cover TagLib# use cases") in the affirmative: the program is MP3/ID3v2-only end to end, uses a small fixed TagLib# surface, and `mutagen` already does the read side of that job in this repo today (`gui/art.py`). TagLib# is not a real constraint on porting - but a port also buys nothing a hardened C#/Python contract wouldn't buy more cheaply, so the recommendation is to keep the split and fix the contract instead. Full analysis: `docs/References/Architecture-Language-Decision.md`. The separate, much cheaper .NET Framework 4.8 -> SDK-style migration question remains open and un-coupled from this - see IDEAS.md.
