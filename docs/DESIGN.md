...
## Binary decisions: one toggle, not a pair of buttons

A per-row/per-card binary choice that has a default gets ONE control showing the state it is in, never two buttons where one is always a no-op. Decided 2026-09-05 for the Integration tab's review card (`.rc-toggle` in `gui/theme.py`, `review_card()` in `gui/tabs/integration.py`): accept/decline used to be a permanent Accept + Decline pair per card, so on a 126-card batch the eye read 252 controls to find the handful that were not in the default state, and undoing a mis-click meant finding the opposite button rather than clicking the same place again. The toggle reads "Accepted" with a tick (calm positive, `--accent2`) or "Declined" with a cross (attention, `--accent4`) and flips on click, carrying `aria-pressed` for assistive tech. The disabled error variant (`.rc-toggle.err`) is the one case that renders as a single fixed state with no flip. Apply the same rule to any future per-item include/exclude, enable/disable or keep/skip control; a pair of buttons is only right when the two options are not each other's complement.

## Destructive keyboard shortcuts live in an explicit, visible mode

A bare letter key that changes state must not be live just because a screen is open. Decided 2026-09-05 alongside the toggle above: the review stage's A (accept) / D (decline) keys were globally armed on stage 2 with nothing on screen mentioning they existed, so a stray keypress could silently flip a decision on a card the user was not even looking at, and the cursor those keys targeted was invisible until keyboard nav had already been used. They are now gated on **Triage mode** - a toolbar button (`icon="keyboard"`, active-filled when on), `T` to toggle, `Esc` to leave - which renders a `.triage-bar` legend of every armed key plus a `Card N of M` position readout, keeps the `.review-card.current` cursor visible, and scrolls it into view. Navigation keys (J/K, arrows) are deliberately exempt and self-arming: they only move a cursor, so pressing one turns the mode on rather than doing nothing, which is what keeps the keyboard path discoverable. The rule for any new shortcut: if the key changes data, it needs a mode that is visible while it is armed and a legend that lists it; if it only moves a cursor or a view, it can stay always-live.
