...
- [ ] **INVESTIGATE: LibChecker duplicate detection bug with featured artists** - BLOCKING CODE BUG. Current duplicate detection compares by title only, ignoring featured artists. Result: "Twista;CeeLo - Hope" flagged as duplicate of "Twista;Faith Evans - Hope" (different songs, same title, different collaborators). The duplicate checker needs to consider ALL artists from TPE1 field, not just title. This causes false positives and prevents proper duplicate detection. High priority - must fix before integration runs can be fully trusted.
- [ ] **Fix LibChecker "feat." and "ft." false positives** - current regex flags these abbreviations anywhere in title/filename, causing false positives when mid-word (e.g., "ft." in "Theft" or "LEFT"). Improve: require whitespace or punctuation before the abbreviation. Blocks clean integration runs. Fix before running LibChecker on full library.
...
- [ ] **Integrator: auto-apply tag cleanup during integration** - integrator must strip unwanted text from NewMusic tracks BEFORE they enter the library, so LibChecker validation passes immediately after integration (zero issues expected). Cleanup rules: remove "(feat. ...)", "(ft. ...)" from Title and Filename; strip "version", "explicit", "edit", "remix" suffixes from Title/Album per exceptions.xml. This is the difference between "manually fix library after integration" and "integrate with zero cleanup needed". Goal: integrate -> run LibChecker -> zero hits. This unblocks the feedback loop.
  - **CRITICAL:** Detection rules (what LibChecker flags) ≠ Removal rules (what integrator removes). Example: LibChecker flags "feat." anywhere in title (to catch accidental inclusions), but integrator should NOT remove all "feat." - some are legitimate featuring credits, some are part of words (false positives). Cleanup list must be explicitly curated per track or per pattern, never blindly auto-remove based on LibChecker hits. When in doubt, flag for human review rather than auto-remove.
