...
- [ ] **Improve LibChecker "feat." and "ft." detection** - current regex flags "feat." and "ft." anywhere in filename/title, causing false positives when these abbreviations appear as part of a word (e.g., "ft." in "LEFT" or "Theft"). Improve: require whitespace or punctuation before the abbreviation - "ft." is only invalid if preceded by a space/punctuation, not when it's mid-word. Reduces false positive exceptions.
