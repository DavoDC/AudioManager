...
- **Post-integration validation always force-regens.** Program.cs calls `AgeChecker.ForceRegen()` before `new Reflector()` in the post-integration block. Any future code that runs Reflector after integration-level file moves/deletes must do the same - incremental Reflector never prunes orphaned XMLs, producing ghost LibChecker hits.

- **Post-integration LibChecker warnings after deletions/moves are probably ghosts - confirm with force-regen before acting.** Post-integration validation regenerates incrementally, which never prunes XMLs for files integration just deleted (`[L]` duplicates) or moved (Misc migration). Those stale XMLs trip duplicate / misc-folder checks against a library state that no longer exists. Fast tell: regen prints `Tags parsed` > `MP3 file count` (the delta = ghost count). Never edit or delete a library file to "resolve" such a duplicate without first running `analysis --force-regen --no-input --no-auto-commit` and re-checking - chasing phantom duplicates risks real data loss. Warnings that survive a force-regen are real. Full mechanism + fixes: `docs/References/Post-Integration-Validation.md`.
